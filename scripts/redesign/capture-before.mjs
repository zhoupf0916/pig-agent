...
try{
 ...
 const r=page.waitForResponse(r=>r.url().endsWith('/api/sessions')&&r.request().method()==='POST');await page.getByRole('button',{name:'新对话',exact:true}).click();const session=await(await r).json();await page.waitForURL('**/sessions/'+session.id);await page.reload();await page.getByLabel('执行位置').selectOption('remote');await page.waitForTimeout(500);if(!await page.getByLabel('写入前审批').isChecked())await page.getByLabel('写入前审批').click();await page.getByPlaceholder(/描述目标/).fill('重设计真实验收：创建并读回 cloud-proof.txt，核验容器成果。');await page.getByRole('button',{name:'发送',exact:true}).click();await page.getByRole('button',{name:'停止',exact:true}).waitFor();await matrix('executing','real mock container; may reach approval during viewport sweep');
