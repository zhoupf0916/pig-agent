...
import { z } from "zod";
import { NetworkAccessError, networkFailure } from "./network-errors.ts";
...
export function publicDnsAddresses(value: unknown): string[] {
  ...
  if (data?.Status !== 0 || !Array.isArray(data.Answer))
    throw Error("公网 DNS 没有返回有效解析结果");
  const addresses = data.Answer.filter((item) => item.type === 1).map(
    (item) => item.data,
  );
  if (!addresses.length || addresses.some((address) => !publicIPv4(address)))
    throw Error("公网 DNS 返回非公网或空地址");
  return addresses;
}
async function publicDnsLookup(
  ...
  const provider = process.env.NETWORK_DNS_PROVIDER || "cloudflare";
  if (!["cloudflare", "alidns"].includes(provider))
    throw Error("NETWORK_DNS_PROVIDER 配置无效");
  const address = provider === "alidns" ? "223.5.5.5" : "1.1.1.1";
  const url = new URL(
    provider === "alidns"
  ...
  return new Promise((resolve, reject) => {
    const req = request(
      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            resolve(publicDnsAddresses(JSON.parse(body)));
          } catch {
            reject(Error("公网 DNS 响应无效或包含非公网地址"));
          }
        });
        res.on("error", reject);
      },
...
export async function resolveNetworkAddresses(
  host: string,
  signal: AbortSignal = new AbortController().signal,
): Promise<string[]> {
  const mode = process.env.NETWORK_DNS_MODE || "system";
  if (!["system", "public"].includes(mode))
    throw Error("NETWORK_DNS_MODE 配置无效");
  const addresses = isIP(host)
    ? [{ address: host, family: 4 }]
    : mode === "public"
      ? (await publicDnsLookup(host, signal)).map((address) => ({
          address,
          family: 4,
        }))
      : await lookup(host, { all: true });
  // Never fall back to a private address; TLS connection pins this vetted IPv4.
  if (addresses.some((a) => a.family === 4 && !publicIPv4(a.address)))
    throw Error("域名解析包含非公网地址，请检查代理或 DNS 配置");
  const address = addresses.find(
    (a) => a.family === 4 && publicIPv4(a.address),
  )?.address;
  if (!address) throw Error("目标没有可用的公网 IPv4 地址");
  return [
    ...new Set(
      addresses
        .filter((a) => a.family === 4 && publicIPv4(a.address))
        .map((a) => a.address),
    ),
  ].slice(0, 4);
}
export async function resolveNetworkAddress(
  ...
  return (await resolveNetworkAddresses(host, signal))[0]!;
}
...
export async function fetchApprovedNetwork(
  ...
  signal = AbortSignal.any([signal, AbortSignal.timeout(input.timeoutMs)]);
  let addresses: string[];
  try {
    addresses = await abortable(
      resolveNetworkAddresses(url.hostname, signal),
      signal,
    );
  } catch (error) {
    throw networkFailure(error, "dns", signal);
  }
    ...
    try {
      return await new Promise<{
        ...
        const req = request(
          ...
          {
            method: "GET",
            // A fresh socket enforces this approval’s pinned DNS and pre-TLS retry boundary.
            agent: false,
            signal,
            family: 4,
            lookup: (_host, _options, callback) => callback(null, address, 4),
      ...
      // Fail over only before a TLS connection is established: never replay an HTTP GET.
      if (
        !connected &&
        !signal.aborted &&
        [
        ...
        continue;
      if (code === "ETIMEDOUT" && !signal.aborted)
        throw new NetworkAccessError(
          "NETWORK_TIMEOUT",
          "connect",
          "目标连接超时；审批已通过，但服务器暂时无法连接目标。",
        );
      throw networkFailure(error, connected ? "response" : "connect", signal);
    }
  }
  throw new NetworkAccessError(
    "NETWORK_UNREACHABLE",
