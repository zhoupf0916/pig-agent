...
- 五个基础包：编码质量、文档写作、数据分析、资料研究、故障排查，每个包含一位专家、一项技能。正文包含输入、步骤、交付与验证要求，可先阅读。
- 停用后不再出现在新任务选择里；移除前须停用。已有任务保留当时的上下文。
- 桌面支持导入原有 `pig-plugin-v1` JSON；云端当前只安装内置目录，不支持上传执行脚本、第三方二进制或自动安装依赖。

新增六个社区精选包：创意构思、需求排期、用户体验研究、交互趣味设计、故事分镜、小游戏策划。各含一项技能与一位专家；用法及固定来源见 [社区精选说明](curated-skills.md)。
