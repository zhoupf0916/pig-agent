...
在工作台「远端运行记录 → 组织与共享项目」创建组织或输入组织邀请码。管理员管理成员和签发一次性、24 小时有效的邀请码；编辑者可创建项目、发起和跟进任务；只读成员可查看会话、日志与成果。组织创建者不能被移除或降级。

共享项目的新任务关联组织权限，同一会话可由不同编辑者串行跟进；移除成员后，他创建的共享任务也不再对他开放。已有私人会话不会自动共享。当前自动化仍按账号隔离，尚未加入共享项目调度和项目文件库。

## 云端操作审批

新建远端会话或共享项目任务时可勾选「写入前审批」。开启后，文件写入、修改、删除、移动、补丁及 shell 命令均先向控制面申请审批，在远端运行记录中查看工具参数并批准或拒绝。已有会话保留原策略，变更策略需新建会话。
