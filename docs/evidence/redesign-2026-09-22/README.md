...
- `final-product/` 仅包含最后一次全新临时数据目录组合冒烟的明确允许清单；管理端16项在2026-09-21T18:55:50.342Z完成。旧flow-debug等中间失败截图不在归档中。本轮flow与F2的sessionId一致；真实重试恢复后额外跟进，建立两个成功版本，保留原双版本断言。
