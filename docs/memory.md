...
- Deleting a project or a session (existing `DELETE`) clears the matching `projectId` / `sessionId` on notes that still named that id. Body / tags stay. An already-open `#/memory` list / detail catches up via the same Milestone AB GET (focus / visibility / short poll) — no ghost link into the deleted project / session, no new poller. Automation `lastSessionId` is not rewritten.
