...
- Inbox: handoff items show the note / asset count; **打开会话** jumps to the workstation while `sessionId` is still set. Another same-host tab refreshes the unread badge and list from `GET /api/inbox` (focus / visibility / short poll). Deleting that session clears `sessionId` on the existing DELETE — the same GET drops the clickable ghost entry. Deleting the project removes inbox rows for that `projectId` (no ghost open-project, no new history model).
