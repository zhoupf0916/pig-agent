...
- **本机搜索 / 记忆（MVP）**：顶栏或 `#/search` 对会话标题 / 近讯、项目名称 / 指令 / 待办 / 动态、资产文件名与文本正文、**钉住笔记 / 回合摘要**做子串检索（无向量库）。同一主机另一标签已打开 `#/search` 且已有查询时，命中列表经只读 `GET /api/search?q=`（焦点 / 可见 / 短轮询）跟上，无需整页刷新；无查询时不强制拉取。顶栏 SearchBox 已有查询且下拉打开时同样跟上；无查询或下拉关闭时不强制拉取。见 [docs/search.md](./search.md)
- **可写本机记忆（MVP）**：`#/memory` 钉住事实或写启发式回合摘要，JSON 在 `data/memory/`。工作台可钉住 / 写摘要。最近若干条钉住会小剂量注入 pig 系统提示。同一主机另一标签已打开该页时，列表经只读 `GET /api/memory`、已打开详情再 `GET /api/memory/:id`（焦点 / 可见 / 短轮询）跟上；另一标签删除该笔记后，已打开详情在列表确认没有该 id 后清掉或切走（不拉 `:id`），无需整页刷新。删除项目 / 会话会清掉仍引用该 id 的记忆 `projectId` / `sessionId`（正文 / 标签不动）；已打开 `#/memory` 经同一 GET 跟上（不留幽灵链接）。见 [docs/memory.md](./memory.md)
...
详见 [交付链路与验收步骤](./delivery-loop.md)。这些增强当前针对 Pig；不改变 Codex / 远程 Cloud 的执行协议。

### 快速开始

环境要求：Node.js 22.13+（建议 24）和 pnpm 11.19.0。仓库通过 `packageManager` 固定 pnpm 版本；`pnpm-workspace.yaml` 显式管理依赖构建权限。
...
| 密钥 | 设置中独立配置；源码模式兼容 `DEEPSEEK_API_KEY` / `CODEX_API_KEY` |
| 工作区信任 | 仅 `realpath(workspaceRoot)`，切换工作区会重写 `[projects."…"]` |
| 中止 | 杀进程组，1.5s 后 SIGKILL |

安装 `@openai/codex` **0.153.4**（或兼容的 CLI），在设置中选择 Codex，发一条「写一个文件」即可看到工具卡片与产物。启动时工作台在现有步骤条上显示中文进度（准备 Codex 环境 → 启动 Codex 进程），首个工具卡片或助手消息后清空。多轮只拼接最近若干条用户/助手文本，**没有 Codex 原生跨轮记忆**。缺少二进制、启动失败或本轮会话出错时给出可读中文原因并可 **重试本轮**，会话回到 idle。

已知缺口（刻意不做）：skills 桥、update_plan/步骤条、细粒度 token 流、danger-full-access。详见 [docs/codex-runtime.md](./codex-runtime.md)。仓库里只有 [examples/codex/](../examples/codex/) 模板，不要提交真实密钥。
...
`local-stub` 让 `pnpm test` 和离线 mock 不需要集群。远程 follow-up 可用 `pnpm mock:cloud`（`http://127.0.0.1:8080`）对着契约冒烟。详见 [docs/cloud-runtime.md](./cloud-runtime.md)。
