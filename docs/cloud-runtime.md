...
4. Later IDLE user turns: `POST /v1/runs/{id}/follow-ups` then subscribe to events again. While follow-up / event-stream reconnect are in flight, the host emits distinct Chinese `steps` chips (继续跟进 → 重新连接事件流) — not the create-run snapshot/create chips. The first inbound stream event clears them (or a Milestone L failure banner).
