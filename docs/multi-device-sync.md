...
AZ already clears asset `sourceSessionId` on `DELETE /api/sessions/:id`. When Tab B already has that asset **preview** open, the same Milestone AA **read-only** `GET /api/projects` plus `GET /api/projects/:id` snapshot also patches the matching open-preview asset fields. Another same-host tab that deletes the source session updates Tab B on focus, visibility, or a short poll — the preview's 「打开来源会话」 control disappears; the preview stays open; the asset file is not deleted; no full page reload; no new poller. Do **not** `GET /api/sessions/:deletedId` (AN-02). Close / reopen preview still uses the existing modal path. Board / list 「来源会话」 still follow Milestone AZ. Open-project-deleted-elsewhere still follows Milestone AP. This does **not** add a write path, a public webhook, or dual-write projects / sessions / `events.jsonl`. Default runtime stays **pig**.

## Open todo highlight after todo delete (Milestone BF)

When Tab B is already on `#/projects/:id?todo=` and that todo is highlighted, the same Milestone AA **read-only** `GET /api/projects` plus `GET /api/projects/:id` snapshot is the source of truth for whether that todo still exists. Another same-host tab that **deletes** the todo updates Tab B on focus, visibility, or a short poll — the board row is already gone via AA; once the detail snapshot lacks that id, strip / clear the `?todo=` highlight. No full page reload; no new poller. Do **not** `GET` the deleted todo id. Optional `?asset=` stays. Open-project-deleted-elsewhere still follows Milestone AP. Board / assets / members while the todo still exists still follow Milestone AA. This does **not** add a write path, a public webhook, or dual-write projects / sessions / `events.jsonl`. Default runtime stays **pig**.
...
## Search hits after target delete (Milestone BC)

When Tab B is already on `#/search` with a query — or the header `SearchBox` already has a query **and** the dropdown is open — the same read-only `GET /api/search?q=` used by Milestones AI / AL is also the source of truth for whether a hit session / project / memory still exists. Another same-host tab that **deletes** that target updates Tab B on focus, visibility, or a short poll — the result set drops that id, no full page reload, no new search index or poller. If Tab B clicks a stale hit **before** that soft refresh and the navigate-to-hit GET returns 404 / gone, drop that row from the current results / dropdown and do **not** open or keep a ghost detail route (AN-02: no GET `:id` after gone). Search write semantics stay the existing paths. This does **not** add a write path, a public webhook, or dual-write search / sessions / `events.jsonl`. Default runtime stays **pig**. Hit list / snapshot JSON never carry provider-key plaintext.
