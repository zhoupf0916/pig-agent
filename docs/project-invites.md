...
- Deleting a project (existing `DELETE`) removes inbox items for that `projectId`. Deleting a session clears `sessionId` on handoff rows (title / body stay). Tab B header inbox follows the same `GET /api/inbox` (focus / visibility / short poll) — no ghost open-project / **打开会话**, no new poller.
- Handoff rows keep **打开会话** while `sessionId` is still set.
