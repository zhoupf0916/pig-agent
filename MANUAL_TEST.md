...
Does **not** change the default runtime (still **本机 Pig**). Deleting a session clears matching project-internal refs (`sourceSessionId` on assets, `sessionId` on todos). Already-open `#/projects` detail is the existing Milestone AA read-only `GET /api/projects` / `:id` catch-up — no clickable 「来源会话」 into the deleted session, no full page reload, no new sync stack. Assets / todos themselves stay. Todo title / status are not rewritten. Activity-feed body text is not rewritten. Inbox / memory / automation / team cleanup stay AX / AV / AW / AY. No new write path, no dual-write of projects / sessions / `events.jsonl`, no public webhook. Not a credentials vault / connector / multi-agent / sandbox change.

- [ ] **AZ-01** Two workstation tabs on the same host. Tab B is on `#/projects` with a project open whose asset has `sourceSessionId`. Tab A deletes that session — Tab B on focus, visibility, or one short poll no longer shows a clickable 「来源会话」 into the deleted session. No full page refresh
- [ ] **AZ-02** After deleting that session, every project todo that pointed at it has `sessionId` cleared. Todo title / status stay
- [ ] **AZ-03** Header chip still defaults to **本机 Pig**. Milestone O–AY / AA·AP / L·M·N / W·X unchanged. Activity feed body text is not rewritten. No dual-write events, no public webhook
- [ ] **AZ-04** Copy / JSON never show `sk-…` / `Bearer` / `DEEPSEEK_API_KEY` in plaintext. Automated: `pnpm test` + `pnpm typecheck`
- [ ] This is **not** a credentials vault, connector, multi-agent parallelism, public webhook, or sandbox change

## Project message sessionId after session delete (Milestone BA)

Does **not** change the default runtime (still **本机 Pig**). Deleting a session clears the structured `sessionId` on project messages (activity / comment / handoff). Already-open `#/projects` detail is the existing Milestone AA read-only `GET /api/projects` / `:id` catch-up — no full page reload, no new sync stack. Messages themselves stay. Body / timestamp / kind are not rewritten. Asset / todo cleanup stays AZ. Inbox / memory / automation / team cleanup stay AX / AV / AW / AY. No new write path, no dual-write of projects / sessions / `events.jsonl`, no public webhook. Not a credentials vault / connector / multi-agent / sandbox change.

- [ ] **BA-01** Two workstation tabs on the same host. Tab B is on `#/projects` with a project open whose activity / comment / handoff still has `sessionId`. Tab A deletes that session — Tab B on focus, visibility, or one short poll no longer has that `sessionId` on those messages. No full page refresh
- [ ] **BA-02** After deleting that session, every project message that pointed at it has `sessionId` cleared. Message `body` / timestamp / kind stay. Messages themselves are not deleted
- [ ] **BA-03** Header chip still defaults to **本机 Pig**. Milestone O–AZ / AA·AP / L·M·N / W·X unchanged. Asset / todo cleanup stays AZ. No dual-write events, no public webhook
...
- [ ] **AW-03** Header chip still defaults to **本机 Pig**. Milestone O–AV / Z·AG·AS·AU / L·M·N / W·X unchanged. No dual-write events, no public webhook
- [ ] **AW-04** Copy / JSON never show `sk-…` / `Bearer` / `DEEPSEEK_API_KEY` in plaintext. Automated: `pnpm test` + `pnpm typecheck`
- [ ] This is **not** a credentials vault, connector, multi-agent parallelism, public webhook, or sandbox change

## Memory refs after project / session delete (Milestone AV)

Does **not** change the default runtime (still **本机 Pig**). Deleting a project / session clears matching memory fields (`projectId` / `sessionId`). Already-open `#/memory` list / detail is the existing Milestone AB read-only `GET /api/memory` catch-up — no clickable ghost entry, no full page reload, no new sync stack. Note body / tags are not rewritten. Automation `lastSessionId` is Milestone AW. Pin / write-summary stay the existing writes. No new write path, no dual-write of memory / sessions / `events.jsonl`, no public webhook. Not a credentials vault / connector / multi-agent / sandbox change.
...
- [ ] **AV-02** Same after deleting a session (`sessionId` cleared). Note body / tags stay. Automation `lastSessionId` is Milestone AW
...
- [ ] **AS-01** Two workstation tabs on the same host. Tab B has an automation open on `#/automations/:id`. Tab A deletes that automation — Tab B on focus, visibility, or one short poll clears or navigates away without a full page refresh. Do not stay on a ghost detail
- [ ] **AS-02** Tab B only reuses existing `GET /api/automations`. After the list lacks that id, do **not** request `:id`. Delete still uses existing `DELETE`. No new write path, no dual-write, no public webhook
- [ ] **AS-03** Header chip still defaults to **本机 Pig**. Milestone O–AR / L·M·N / W·X / Z·AG unchanged. 「立即运行」 / cron / pin semantics unchanged
- [ ] **AS-04** Copy / JSON never show `sk-…` / `Bearer` / `DEEPSEEK_API_KEY` in plaintext. Automated: `pnpm test` + `pnpm typecheck`
- [ ] This is **not** a credentials vault, connector, multi-agent parallelism, public webhook, or sandbox change
