...
- **本机专家 / playbook（MVP）**：`#/experts` 目录 + 工作台钉选。内置侦察 / 规划 / 实现 / 评审；可引用 `skills/`。**专家指令先于项目指令**注入 pig / Codex / cloud-stub。`mode=chain` 小队在同一会话里顺序各跑一轮（`session.teamRun` + **顺序执行小队**）。同一主机另一标签已打开该页时，列表经只读 `GET /api/experts` 与 `GET /api/expert-teams`、已打开详情再 `GET /api/experts/:id`（焦点 / 可见 / 短轮询）跟上；另一标签删除该专家后，已打开详情在列表确认没有该 id 后清掉或切走（不拉 `:id`），无需整页刷新。工作台钉选下拉的专家 / 小队目录经同一对 GET 跟上。见 [docs/experts.md](./docs/experts.md)
