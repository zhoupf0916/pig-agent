...
- **本机搜索 / 记忆（MVP）**：顶栏或 `#/search` 对会话标题 / 近讯、项目名称 / 指令 / 待办 / 动态、资产文件名与文本正文、**钉住笔记 / 回合摘要**做子串检索（无向量库）。同一主机另一标签已打开 `#/search` 且已有查询时，命中列表经只读 `GET /api/search?q=`（焦点 / 可见 / 短轮询）跟上，无需整页刷新；无查询时不强制拉取。见 [docs/search.md](./docs/search.md)
